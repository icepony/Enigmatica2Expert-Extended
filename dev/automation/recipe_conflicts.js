...
export async function init(h = defaultHelper) {
  ...
  const unfilteredConflicts = [
    ...subLog.matchAll(
      /^\[SERVER_STARTED\]\[SERVER\]\[INFO\] (\[(Shaped|Shapeless)\] .*)$/gim
    ),
  ]
...
const fineConflicts = [
  ...
  (r1, r2) => r2.name === 'crafttweaker:oredicted_slab',
...
function handleConflict(match) {
  ...
  const [, rec1_name, rec1_item, rec1_out, rec2_name, rec2_item, rec2_out]
    = match.match(rgx)
