#modloaded ae2fc

// Replaced by natie AE2 Fluid Assembler
utils.rh(<ae2fc:fluid_assembler>);

// [ME Fluid Discretizer] from [64k ME Fluid Storage Component][+4]
craft.remake(<ae2fc:fluid_discretizer>, ['pretty',
  '▬ E ▬',
  'M 6 S',
  '▬ E ▬'], {
  'S': <appliedenergistics2:part:220>, // ME Storage Bus
  'E': <appliedenergistics2:material:24>, // Engineering Processor
  '6': <appliedenergistics2:material:57>, // 64k ME Fluid Storage Component
  '▬': <ore:ingotFakeIron>, // Iron Alloy Ingot
  'M': <appliedenergistics2:part:221>, // ME Fluid Storage Bus
});
