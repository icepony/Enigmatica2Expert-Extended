...
/* Inject_js{
val toCot = (tm) => tm.replace('<', '<item:')
val result = getOreBases_byKinds(['ore', 'dust'])
  .filter((b) => b !== 'Aluminum')
  .flatMap((base) => {
    val dustStack = getByOredict_first('dust' + base).commandString
...
function onFalling(armor as IItemStack, player as IPlayer) as void {
  ...
  val isVerbose = !isNull(player.currentItem)
  ...
  val verticalSpeedChance = 1.0 - 1.0 / (abs(motionY) + 1.0);
  if (isVerbose) player.sendMessage('§8Chance to break:§7 ' ~ (verticalSpeedChance * 100.0) as int ~ '%');
  ...
  val amount = max(1.0, (power as double / 12.0) * outStack.amount as double) as int;
